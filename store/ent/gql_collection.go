	...
	for _, field := range graphql.CollectFields(opCtx, collected.Selections, satisfies) {
		...
		case "wonTenders":
			...
			if hasCollectedField(ctx, append(path, totalCountField)...) || hasCollectedField(ctx, append(path, pageInfoField)...) {
				...
				if hasPagination || ignoredEdges {
					query := query.Clone()
					c.loadTotal = append(c.loadTotal, func(ctx context.Context, nodes []*Competitor) error {
						ids := make([]driver.Value, len(nodes))
						for i := range nodes {
							ids[i] = nodes[i].ID
						}
						var v []struct {
							NodeID xid.ID `sql:"competitor_id"`
							Count  int    `sql:"count"`
						}
						query.Where(func(s *sql.Selector) {
							s.Where(sql.InValues(s.C(competitor.WonTendersColumn), ids...))
						})
						if err := query.GroupBy(competitor.WonTendersColumn).Aggregate(Count()).Scan(ctx, &v); err != nil {
							return err
						}
						m := make(map[xid.ID]int, len(v))
						for i := range v {
					...
					c.loadTotal = append(c.loadTotal, func(_ context.Context, nodes []*Competitor) error {
						for i := range nodes {
							n := len(nodes[i].Edges.WonTenders)
							if nodes[i].Edges.totalCount[0] == nil {
								nodes[i].Edges.totalCount[0] = make(map[string]int)
							}
							nodes[i].Edges.totalCount[0][alias] = n
			...
			if limit := paginateLimit(args.first, args.last); limit > 0 {
				if oneNode {
					...
					modify := entgql.LimitPerRow(competitor.WonTendersColumn, limit, pager.orderExpr(query))
					query.modifiers = append(query.modifiers, modify)
				}
			} else {
				query = pager.applyOrder(query)
			}
			c.WithNamedWonTenders(alias, func(wq *TenderQuery) {
				*wq = *query
			})
		case "createdAt":
