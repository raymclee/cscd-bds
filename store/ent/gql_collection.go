...
func (c *CompetitorQuery) collectField(ctx context.Context, oneNode bool, opCtx *graphql.OperationContext, collected graphql.CollectedField, path []string, satisfies ...string) error {
	...
	for _, field := range graphql.CollectFields(opCtx, collected.Selections, satisfies) {
		...
		case "tenders":
			...
			c.WithNamedTenders(alias, func(wq *TenderCompetitorQuery) {
				*wq = *query
			})
		case "createdAt":
...
func (c *CustomerQuery) collectField(ctx context.Context, oneNode bool, opCtx *graphql.OperationContext, collected graphql.CollectedField, path []string, satisfies ...string) error {
	...
	for _, field := range graphql.CollectFields(opCtx, collected.Selections, satisfies) {
		...
		case "updatedBy":
			...
			if _, ok := fieldSeen[customer.FieldUpdatedByID]; !ok {
				selectedFields = append(selectedFields, customer.FieldUpdatedByID)
				fieldSeen[customer.FieldUpdatedByID] = struct{}{}
			}
		...
		case "profiles":
			...
			args := newCustomerProfilePaginateArgs(fieldArgs(ctx, new(CustomerProfileWhereInput), path...))
			if err := validateFirstLast(args.first, args.last); err != nil {
				return fmt.Errorf("validate first and last in path %q: %w", path, err)
			}
			pager, err := newCustomerProfilePager(args.opts, args.last != nil)
			if err != nil {
				return fmt.Errorf("create new pager in path %q: %w", path, err)
			}
			if query, err = pager.applyFilter(query); err != nil {
			...
			if hasCollectedField(ctx, append(path, totalCountField)...) || hasCollectedField(ctx, append(path, pageInfoField)...) {
				...
				if hasPagination || ignoredEdges {
					...
					c.loadTotal = append(c.loadTotal, func(ctx context.Context, nodes []*Customer) error {
						...
						query.Where(func(s *sql.Selector) {
							s.Where(sql.InValues(s.C(customer.ProfilesColumn), ids...))
						})
						if err := query.GroupBy(customer.ProfilesColumn).Aggregate(Count()).Scan(ctx, &v); err != nil {
							return err
						}
						m := make(map[xid.ID]int, len(v))
						for i := range v {
			...
			path = append(path, edgesField, nodeField)
			if field := collectedField(ctx, path...); field != nil {
				if err := query.collectField(ctx, false, opCtx, *field, path, mayAddCondition(satisfies, customerprofileImplementors)...); err != nil {
					return err
				}
			}
			if limit := paginateLimit(args.first, args.last); limit > 0 {
				if oneNode {
					...
					modify := entgql.LimitPerRow(customer.ProfilesColumn, limit, pager.orderExpr(query))
					query.modifiers = append(query.modifiers, modify)
				}
			} else {
				query = pager.applyOrder(query)
			}
			c.WithNamedProfiles(alias, func(wq *CustomerProfileQuery) {
				*wq = *query
			})
		...
		case "activeProfile":
			...
			if err := query.collectField(ctx, oneNode, opCtx, field, path, mayAddCondition(satisfies, customerprofileImplementors)...); err != nil {
				return err
			}
			c.withActiveProfile = query
			if _, ok := fieldSeen[customer.FieldActiveProfileID]; !ok {
				selectedFields = append(selectedFields, customer.FieldActiveProfileID)
				fieldSeen[customer.FieldActiveProfileID] = struct{}{}
			}
		...
		case "pendingProfile":
			...
			c.withPendingProfile = query
			if _, ok := fieldSeen[customer.FieldPendingProfileID]; !ok {
				selectedFields = append(selectedFields, customer.FieldPendingProfileID)
				fieldSeen[customer.FieldPendingProfileID] = struct{}{}
			}
		case "createdAt":
		...
		case "approvalStatus":
			if _, ok := fieldSeen[customer.FieldApprovalStatus]; !ok {
				selectedFields = append(selectedFields, customer.FieldApprovalStatus)
				fieldSeen[customer.FieldApprovalStatus] = struct{}{}
			}
		case "ownerType":
...
// CollectFields tells the query-builder to eagerly load connected nodes by resolver context.
func (cp *CustomerProfileQuery) CollectFields(ctx context.Context, satisfies ...string) (*CustomerProfileQuery, error) {
	fc := graphql.GetFieldContext(ctx)
	if fc == nil {
		return cp, nil
	}
...
func (cp *CustomerProfileQuery) collectField(ctx context.Context, oneNode bool, opCtx *graphql.OperationContext, collected graphql.CollectedField, path []string, satisfies ...string) error {
	path = append([]string(nil), path...)
	var (
		unknownSeen    bool
		fieldSeen      = make(map[string]struct{}, len(customerprofile.Columns))
		selectedFields = []string{customerprofile.FieldID}
	)
	for _, field := range graphql.CollectFields(opCtx, collected.Selections, satisfies) {
		switch field.Name {
		...
		case "customer":
			var (
				alias = field.Alias
				path  = append(path, alias)
				query = (&CustomerClient{config: cp.config}).Query()
			)
			if err := query.collectField(ctx, oneNode, opCtx, field, path, mayAddCondition(satisfies, customerImplementors)...); err != nil {
				return err
			}
			cp.withCustomer = query
			if _, ok := fieldSeen[customerprofile.FieldCustomerID]; !ok {
				selectedFields = append(selectedFields, customerprofile.FieldCustomerID)
				fieldSeen[customerprofile.FieldCustomerID] = struct{}{}
			}
		...
		case "createdBy":
			...
			cp.withCreatedBy = query
			if _, ok := fieldSeen[customerprofile.FieldCreatedByID]; !ok {
				selectedFields = append(selectedFields, customerprofile.FieldCreatedByID)
				fieldSeen[customerprofile.FieldCreatedByID] = struct{}{}
			}
		...
		case "approver":
			...
			cp.withApprover = query
			if _, ok := fieldSeen[customerprofile.FieldApproverID]; !ok {
				selectedFields = append(selectedFields, customerprofile.FieldApproverID)
				fieldSeen[customerprofile.FieldApproverID] = struct{}{}
			}
		...
		case "sales":
			...
			cp.withSales = query
			if _, ok := fieldSeen[customerprofile.FieldSalesID]; !ok {
				selectedFields = append(selectedFields, customerprofile.FieldSalesID)
				fieldSeen[customerprofile.FieldSalesID] = struct{}{}
			}
		case "createdAt":
			if _, ok := fieldSeen[customerprofile.FieldCreatedAt]; !ok {
				selectedFields = append(selectedFields, customerprofile.FieldCreatedAt)
				fieldSeen[customerprofile.FieldCreatedAt] = struct{}{}
			}
		case "updatedAt":
			if _, ok := fieldSeen[customerprofile.FieldUpdatedAt]; !ok {
				selectedFields = append(selectedFields, customerprofile.FieldUpdatedAt)
				fieldSeen[customerprofile.FieldUpdatedAt] = struct{}{}
			}
		case "name":
			if _, ok := fieldSeen[customerprofile.FieldName]; !ok {
				selectedFields = append(selectedFields, customerprofile.FieldName)
				fieldSeen[customerprofile.FieldName] = struct{}{}
			}
		case "approvalStatus":
			if _, ok := fieldSeen[customerprofile.FieldApprovalStatus]; !ok {
		...
		case "ownerType":
			if _, ok := fieldSeen[customerprofile.FieldOwnerType]; !ok {
				...
				fieldSeen[customerprofile.FieldOwnerType] = struct{}{}
			}
		case "industry":
			if _, ok := fieldSeen[customerprofile.FieldIndustry]; !ok {
				selectedFields = append(selectedFields, customerprofile.FieldIndustry)
				fieldSeen[customerprofile.FieldIndustry] = struct{}{}
			}
		case "size":
			if _, ok := fieldSeen[customerprofile.FieldSize]; !ok {
				selectedFields = append(selectedFields, customerprofile.FieldSize)
				fieldSeen[customerprofile.FieldSize] = struct{}{}
			}
		case "contactPerson":
			if _, ok := fieldSeen[customerprofile.FieldContactPerson]; !ok {
				selectedFields = append(selectedFields, customerprofile.FieldContactPerson)
				fieldSeen[customerprofile.FieldContactPerson] = struct{}{}
			}
		case "contactPersonPosition":
			if _, ok := fieldSeen[customerprofile.FieldContactPersonPosition]; !ok {
				selectedFields = append(selectedFields, customerprofile.FieldContactPersonPosition)
				fieldSeen[customerprofile.FieldContactPersonPosition] = struct{}{}
			}
		case "contactPersonPhone":
			if _, ok := fieldSeen[customerprofile.FieldContactPersonPhone]; !ok {
				selectedFields = append(selectedFields, customerprofile.FieldContactPersonPhone)
				fieldSeen[customerprofile.FieldContactPersonPhone] = struct{}{}
			}
		case "contactPersonEmail":
			if _, ok := fieldSeen[customerprofile.FieldContactPersonEmail]; !ok {
				selectedFields = append(selectedFields, customerprofile.FieldContactPersonEmail)
				fieldSeen[customerprofile.FieldContactPersonEmail] = struct{}{}
			}
		case "salesID":
			if _, ok := fieldSeen[customerprofile.FieldSalesID]; !ok {
				selectedFields = append(selectedFields, customerprofile.FieldSalesID)
...
type customerprofilePaginateArgs struct {
	first, last   *int
	after, before *Cursor
	opts          []CustomerProfilePaginateOption
}

func newCustomerProfilePaginateArgs(rv map[string]any) *customerprofilePaginateArgs {
	args := &customerprofilePaginateArgs{}
	if rv == nil {
		return args
	}
	if v := rv[firstField]; v != nil {
	...
	if v, ok := rv[orderByField]; ok {
		switch v := v.(type) {
		case []*CustomerProfileOrder:
			args.opts = append(args.opts, WithCustomerProfileOrder(v))
		case []any:
			var orders []*CustomerProfileOrder
			for i := range v {
				mv, ok := v[i].(map[string]any)
				if !ok {
					continue
				...
				var (
					err1, err2 error
					order      = &CustomerProfileOrder{Field: &CustomerProfileOrderField{}, Direction: entgql.OrderDirectionAsc}
				)
				if d, ok := mv[directionField]; ok {
					err1 = order.Direction.UnmarshalGQL(d)
				}
			...
			args.opts = append(args.opts, WithCustomerProfileOrder(orders))
		}
	}
	if v, ok := rv[whereField].(*CustomerProfileWhereInput); ok {
		args.opts = append(args.opts, WithCustomerProfileFilter(v.Filter))
	}
	return args
}
...
func (pr *ProjectQuery) collectField(ctx context.Context, oneNode bool, opCtx *graphql.OperationContext, collected graphql.CollectedField, path []string, satisfies ...string) error {
	...
	for _, field := range graphql.CollectFields(opCtx, collected.Selections, satisfies) {
		...
		case "code":
			if _, ok := fieldSeen[project.FieldCode]; !ok {
...
func (t *TenderQuery) collectField(ctx context.Context, oneNode bool, opCtx *graphql.OperationContext, collected graphql.CollectedField, path []string, satisfies ...string) error {
	...
	for _, field := range graphql.CollectFields(opCtx, collected.Selections, satisfies) {
		...
		case "profiles":
			...
			args := newTenderProfilePaginateArgs(fieldArgs(ctx, new(TenderProfileWhereInput), path...))
			if err := validateFirstLast(args.first, args.last); err != nil {
				return fmt.Errorf("validate first and last in path %q: %w", path, err)
			}
			pager, err := newTenderProfilePager(args.opts, args.last != nil)
			if err != nil {
				return fmt.Errorf("create new pager in path %q: %w", path, err)
			}
			if query, err = pager.applyFilter(query); err != nil {
			...
			if hasCollectedField(ctx, append(path, totalCountField)...) || hasCollectedField(ctx, append(path, pageInfoField)...) {
				...
				if hasPagination || ignoredEdges {
					...
					t.loadTotal = append(t.loadTotal, func(ctx context.Context, nodes []*Tender) error {
						...
						query.Where(func(s *sql.Selector) {
							s.Where(sql.InValues(s.C(tender.ProfilesColumn), ids...))
						})
						if err := query.GroupBy(tender.ProfilesColumn).Aggregate(Count()).Scan(ctx, &v); err != nil {
							return err
						}
						m := make(map[xid.ID]int, len(v))
						for i := range v {
			...
			if limit := paginateLimit(args.first, args.last); limit > 0 {
				if oneNode {
					...
					modify := entgql.LimitPerRow(tender.ProfilesColumn, limit, pager.orderExpr(query))
					query.modifiers = append(query.modifiers, modify)
				}
			} else {
				query = pager.applyOrder(query)
		...
		case "approvalStatus":
			if _, ok := fieldSeen[tender.FieldApprovalStatus]; !ok {
				selectedFields = append(selectedFields, tender.FieldApprovalStatus)
				fieldSeen[tender.FieldApprovalStatus] = struct{}{}
			}
		case "approvalMsgID":
			if _, ok := fieldSeen[tender.FieldApprovalMsgID]; !ok {
				selectedFields = append(selectedFields, tender.FieldApprovalMsgID)
				fieldSeen[tender.FieldApprovalMsgID] = struct{}{}
			}
		case "name":
...
// CollectFields tells the query-builder to eagerly load connected nodes by resolver context.
func (tc *TenderCompetitorQuery) CollectFields(ctx context.Context, satisfies ...string) (*TenderCompetitorQuery, error) {
	fc := graphql.GetFieldContext(ctx)
	if fc == nil {
		return tc, nil
	}
...
func (tc *TenderCompetitorQuery) collectField(ctx context.Context, oneNode bool, opCtx *graphql.OperationContext, collected graphql.CollectedField, path []string, satisfies ...string) error {
	path = append([]string(nil), path...)
	var (
		unknownSeen    bool
		fieldSeen      = make(map[string]struct{}, len(tendercompetitor.Columns))
		selectedFields = []string{tendercompetitor.FieldID}
	)
	for _, field := range graphql.CollectFields(opCtx, collected.Selections, satisfies) {
		switch field.Name {
		...
		case "tender":
			var (
				alias = field.Alias
				path  = append(path, alias)
				query = (&TenderClient{config: tc.config}).Query()
			)
			if err := query.collectField(ctx, oneNode, opCtx, field, path, mayAddCondition(satisfies, tenderImplementors)...); err != nil {
				return err
			}
			tc.withTender = query
			if _, ok := fieldSeen[tendercompetitor.FieldTenderID]; !ok {
				selectedFields = append(selectedFields, tendercompetitor.FieldTenderID)
				fieldSeen[tendercompetitor.FieldTenderID] = struct{}{}
			}
		...
		case "competitor":
			var (
				alias = field.Alias
				path  = append(path, alias)
				query = (&CompetitorClient{config: tc.config}).Query()
			)
			if err := query.collectField(ctx, oneNode, opCtx, field, path, mayAddCondition(satisfies, competitorImplementors)...); err != nil {
				return err
			}
			tc.withCompetitor = query
			if _, ok := fieldSeen[tendercompetitor.FieldCompetitorID]; !ok {
				selectedFields = append(selectedFields, tendercompetitor.FieldCompetitorID)
				fieldSeen[tendercompetitor.FieldCompetitorID] = struct{}{}
			}
		case "createdAt":
			if _, ok := fieldSeen[tendercompetitor.FieldCreatedAt]; !ok {
				selectedFields = append(selectedFields, tendercompetitor.FieldCreatedAt)
				fieldSeen[tendercompetitor.FieldCreatedAt] = struct{}{}
			}
		case "updatedAt":
			if _, ok := fieldSeen[tendercompetitor.FieldUpdatedAt]; !ok {
				selectedFields = append(selectedFields, tendercompetitor.FieldUpdatedAt)
				fieldSeen[tendercompetitor.FieldUpdatedAt] = struct{}{}
			}
		case "tenderID":
			if _, ok := fieldSeen[tendercompetitor.FieldTenderID]; !ok {
				selectedFields = append(selectedFields, tendercompetitor.FieldTenderID)
		...
		case "amount":
			if _, ok := fieldSeen[tendercompetitor.FieldAmount]; !ok {
...
type tendercompetitorPaginateArgs struct {
	first, last   *int
	after, before *Cursor
	opts          []TenderCompetitorPaginateOption
}

func newTenderCompetitorPaginateArgs(rv map[string]any) *tendercompetitorPaginateArgs {
	args := &tendercompetitorPaginateArgs{}
	if rv == nil {
		return args
	}
	if v := rv[firstField]; v != nil {
	...
	if v, ok := rv[orderByField]; ok {
		switch v := v.(type) {
		case []*TenderCompetitorOrder:
			args.opts = append(args.opts, WithTenderCompetitorOrder(v))
		case []any:
			var orders []*TenderCompetitorOrder
			for i := range v {
				mv, ok := v[i].(map[string]any)
				if !ok {
					continue
				...
				var (
					err1, err2 error
					order      = &TenderCompetitorOrder{Field: &TenderCompetitorOrderField{}, Direction: entgql.OrderDirectionAsc}
				)
				if d, ok := mv[directionField]; ok {
					err1 = order.Direction.UnmarshalGQL(d)
				}
			...
			args.opts = append(args.opts, WithTenderCompetitorOrder(orders))
		}
	}
...
func newTenderProfilePaginateArgs(rv map[string]any) *tenderprofilePaginateArgs {
	...
	if v, ok := rv[orderByField]; ok {
		...
		case []any:
			...
			args.opts = append(args.opts, WithTenderProfileOrder(orders))
		}
	}
...
func (u *UserQuery) collectField(ctx context.Context, oneNode bool, opCtx *graphql.OperationContext, collected graphql.CollectedField, path []string, satisfies ...string) error {
	...
	for _, field := range graphql.CollectFields(opCtx, collected.Selections, satisfies) {
		...
		case "isCeo":
			if _, ok := fieldSeen[user.FieldIsCeo]; !ok {
				selectedFields = append(selectedFields, user.FieldIsCeo)
				fieldSeen[user.FieldIsCeo] = struct{}{}
			}
		case "isSuperAdmin":
