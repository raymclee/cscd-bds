...
func (cq *CountryQuery) QueryProvinces() *ProvinceQuery {
...
func (cq *CountryQuery) FirstID(ctx context.Context) (id xid.ID, err error) {
	var ids []xid.ID
...
func (cq *CountryQuery) FirstIDX(ctx context.Context) xid.ID {
...
func (cq *CountryQuery) OnlyID(ctx context.Context) (id xid.ID, err error) {
	var ids []xid.ID
...
func (cq *CountryQuery) OnlyIDX(ctx context.Context) xid.ID {
...
func (cq *CountryQuery) IDs(ctx context.Context) (ids []xid.ID, err error) {
...
func (cq *CountryQuery) IDsX(ctx context.Context) []xid.ID {
	ids, err := cq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (cq *CountryQuery) WithProvinces(opts ...func(*ProvinceQuery)) *CountryQuery {
	query := (&ProvinceClient{config: cq.config}).Query()
...
func (cq *CountryQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Country, error) {
	...
	if query := cq.withProvinces; query != nil {
		if err := cq.loadProvinces(ctx, query, nodes,
			func(n *Country) { n.Edges.Provinces = []*Province{} },
			func(n *Country, e *Province) { n.Edges.Provinces = append(n.Edges.Provinces, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range cq.withNamedProvinces {
		if err := cq.loadProvinces(ctx, query, nodes,
			func(n *Country) { n.appendNamedProvinces(name) },
			func(n *Country, e *Province) { n.appendNamedProvinces(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (cq *CountryQuery) loadProvinces(ctx context.Context, query *ProvinceQuery, nodes []*Country, init func(*Country), assign func(*Country, *Province)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[xid.ID]*Country)
	for i := range nodes {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(province.FieldCountryID)
	}
	query.Where(predicate.Province(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(country.ProvincesColumn), fks...))
	}))
...
func (cq *CountryQuery) WithNamedProvinces(name string, opts ...func(*ProvinceQuery)) *CountryQuery {
	query := (&ProvinceClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if cq.withNamedProvinces == nil {
		cq.withNamedProvinces = make(map[string]*ProvinceQuery)
	}
	cq.withNamedProvinces[name] = query
	return cq
}
